...
- **Caméra du vol (mode dos) repliée dans la `transform`** — `perspective-origin` n'est plus animée : son effet passe par un cisaillement en z (`matrix3d` en tête de liste, coefficients `--pf-cam-x`/`--pf-cam-y` posés par `bookshelf.js`). C'est la seule propriété du vol non compositable ; animée, elle avançait sur l'horloge du fil principal pendant que la `transform` avançait sur celle du compositeur → tremblement mesuré sur mobile. Équivalence exacte, vérifiée à 0,00 px sur 768 valeurs de géométrie.
