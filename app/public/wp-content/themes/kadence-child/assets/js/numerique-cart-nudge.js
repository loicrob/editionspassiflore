...
( function ( wp ) {
	...
	// Même infobulle que la fiche livre (composant .pf-tooltip / pf-tooltip.js).
	function tipMarkup( tipText, id ) {
		if ( ! tipText ) {
			return '';
		}
		return '<span class="pf-tooltip"><span class="pf-tooltip__trigger" tabindex="0" role="button" aria-label="Détails de l\'offre numérique" aria-describedby="' + id + '">' +
			...
			'</span><span class="pf-tooltip__bubble" id="' + id + '" role="tooltip">' + escapeHtml( tipText ) + '</span></span>';
	}
	...
	function render( offers ) {
		...
		if ( window.pfTooltip ) {
			var tips = box.querySelectorAll( '.pf-tooltip' );
