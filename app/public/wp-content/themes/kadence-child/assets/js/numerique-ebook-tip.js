...
( function ( wp ) {
	...
	// Même infobulle que la fiche livre / l'encart panier (composant .pf-numerique-tip / pf-tooltip.js).
	function tipMarkup( id ) {
		return '<span class="pf-numerique-tip"><span class="pf-numerique-tip__trigger" tabindex="0" role="button" aria-label="Précision sur le téléchargement" aria-describedby="' + id + '">' +
			...
			'</span><span class="pf-numerique-tip__bubble" id="' + id + '" role="tooltip">Vous pourrez le télécharger en ePub une fois la commande effectuée.</span></span>';
	}
	...
	function apply() {
		...
		items.forEach( function ( item, i ) {
			...
			if ( ! priceCell || priceCell.querySelector( '.pf-numerique-tip' ) ) {
				return;
			}
			priceCell.insertAdjacentHTML( 'beforeend', tipMarkup( 'pf-numerique-tip-bubble-ebook-' + item.id ) );
			if ( window.pfTooltip ) {
				window.pfTooltip.wire( priceCell.querySelector( '.pf-numerique-tip' ) );
			}
		} );
	}
