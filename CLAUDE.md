...
- `Leaflet.markercluster` : regroupe au dézoom, **clic sur cluster ouvre une infobulle combinée** (pas de zoom) groupée par dénominateur commun (ville/dept/région).
...
- **Géocodage (Nominatim) côté écriture uniquement** : à l'enregistrement du lieu + backfill WP-Cron auto-drainant (5/lot). Cache `_pf_venue_lat/lng/geo_src` (stampé même en échec, anti-boucle). Throttle ≥1,1s/appel. Front = zéro appel réseau (hors chargement des tuiles OSM par le navigateur — RGPD documenté dans la politique de confidentialité).
